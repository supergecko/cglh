...
<script>
	export default {
		...
		onLoad(option){
			...
			this.to_mobile = option.to_mobile
		...
		methods: {
			...
			_drawing(){
				...
				const to_mobile = this.to_mobile
				const number = this.number
				const verification_code = this.verification_code
				const timestamp = Date.parse(new Date()) / 1000
				// const memo = this.memo
				const sign = this.$md5(`${mobile}**${currency_id}**${number}**${to_mobile}**${verification_code}**${timestamp}**cglhhaofengshui`)
				let params = {currency_id, to_mobile, number, verification_code, timestamp, sign}
				this.$ajax.post({
					url: this.$service.api_lists.transfer_assets,
					data: params
				}).then((res) => {
					if (res.data.code == 1) {
						//初始化信息
