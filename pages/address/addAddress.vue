<template>
	<view class="wrap">
		...
		<view class="but" @click="_addCoinAddress">添加</view>
	</view>
</template>
	...
	export default{
		...
		methods:{
			...
			_addCoinAddress() {
				...
				const timestamp = Date.parse(new Date()) / 1000
				const sign = this.$md5(`${mobile}**${currency_id}**${address}**${bank_memo}**${description}**${timestamp}**cglhhaofengshui`)
				let params = {
					currency_id,
					address,
					bank_memo,
				...
				this.$ajax.post({
					url: this.$service.api_lists.addCoinAddress,
					data: params
				}).then((res) => {
					if (res.data.code == 1) {
						//初始化信息
