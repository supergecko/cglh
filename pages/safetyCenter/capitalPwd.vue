<template>
	<view class="wrap">
		...
		<view class="but" @click="_securityPasswordSet">设置</view>
	</view>
</template>
	...
	export default{
		...
		methods:{
			...
			_securityPasswordSet() {
				...
				const sign = this.$md5(`${mobile}**${password}**${confirm_password}**${timestamp}**cglhhaofengshui`)
				let params = {password, confirm_password, timestamp, sign}
				this.$ajax.post({
					url: this.$service.api_lists.securityPasswordSet,
					data: params
				}).then((res) => {
					if (res.data.code == 1) {
						//初始化信息
